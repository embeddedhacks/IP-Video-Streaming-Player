...
typedef int(WINAPI *pvsnetchancallback)(const char *sername, const char *url, WORD port,
										...
										int channels, int ch, BOOL bLogin, LONG hChHandle);
...
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientStartup(UINT m_nMessage,HWND m_hWnd,void (WINAPI *m_messagecallback)(LONG hHandle,int wParam,int lParam,void *context) = NULL,void *context = NULL, char *key = NULL);
...
CNET_APIIMPORT BOOL	 __stdcall VSNET_ClientPTZCtrl(LONG hHandle,int type,int value,int priority = 0);
...
//talk bothway
CNET_APIIMPORT LONG  __stdcall VSNET_ClientTalkBothwayStart(char *m_url,char m_ch,char *m_username,char *m_password,WORD wserport,void(WINAPI *TalkCallBack)(char *pbuff,int size),char *m_servername,HWND hwnd);
